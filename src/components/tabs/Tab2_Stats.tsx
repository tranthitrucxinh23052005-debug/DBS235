...
import { Users, TrendingUp, Award, CheckCircle, ChevronDown, ChevronUp, Sparkles, Info, HelpCircle } from 'lucide-react';
import {
...
const LEVEL_COLORS: Record<string, string> = {
  'Xuất sắc': '#10b981', // Emerald 500
  'Giỏi': '#3b82f6',    // Blue 500
  'Khá': '#f59e0b',     // Amber 500
  'Trung bình': '#6366f1', // Indigo 500
  'Yếu': '#f43f5e',      // Rose 500
  'Không đạt': '#94a3b8', // Slate 400
};
  ...
  const analyzeBoxplot = async () => {
    ...
    try {
      const png = await toPng(boxRef.current, { 
        backgroundColor: '#ffffff', // BẮT BUỘC có nền trắng để AI đọc được
        pixelRatio: 2 
      });
      ...
      // Key phải là 'file' để khớp với tham số 'file: UploadFile' ở Backend
      formData.append('file', blob, 'boxplot.png'); 
      formData.append('chart_type', 'boxplot');

      const res = await apiChartInsight(formData);
      
      // Nếu Backend trả về lỗi bên trong JSON
      if (res.status === 'error') {
        setAiInsight("Lỗi Backend: " + res.message);
      } else {
        setAiInsight(res.insight || res.ai_report || "AI không trả về kết quả.");
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 overflow-hidden" ref={boxRef}>
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1 italic">Trực quan hóa độ biến thiên và các giá trị ngoại lệ của từng môn học</p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
              <div className="space-y-4">
                <div className="flex gap-4">
                  ...
                  <p className="text-sm text-slate-600"><strong className="text-slate-800">Đường giữa hộp (Median):</strong> Cho thấy giá trị trung vị. 50% sinh viên có điểm cao hơn mức này và 50% thấp hơn.</p>
                </div>
                <div className="flex gap-4">
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-2 shrink-0" />
                  <p className="text-sm text-slate-600"><strong className="text-slate-800">Chiều cao hộp (IQR):</strong> Hộp càng cao thì điểm số của nhóm 50% ở giữa càng biến động mạnh.</p>
                </div>
              </div>
              <div className="space-y-4">
                <div className="flex gap-4">
                  <div className="w-1.5 h-1.5 rounded-full bg-red-500 mt-2 shrink-0" />
                  <p className="text-sm text-slate-600"><strong className="text-slate-800">Đường đỏ (Mean):</strong> Điểm trung bình cộng. Nếu cách xa Median, dữ liệu đang bị lệch.</p>
                </div>
                <div className="flex gap-4">
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-2 shrink-0" />
                  <p className="text-sm text-slate-600"><strong className="text-slate-800">Râu (Whiskers):</strong> Thể hiện phạm vi điểm bình thường. Các chấm nằm ngoài râu là sinh viên có điểm đột biến.</p>
                </div>
              </div>
            </div>
          </div>
...
function BoxTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white border-none rounded-2xl shadow-xl p-5 text-xs min-w-[200px] space-y-2">
      <p className="font-bold text-slate-800 border-b pb-2 mb-2 text-sm">{label}</p>
      <div className="flex justify-between text-slate-500"><span>Cao nhất (Max):</span> <span className="font-bold text-slate-800">{d.max}</span></div>
      <div className="flex justify-between text-slate-500"><span>Q3 (75%):</span> <span className="font-bold text-slate-800">{d.q3}</span></div>
      <div className="flex justify-between text-slate-500"><span>Trung vị (Median):</span> <span className="font-bold text-blue-600">{d.median}</span></div>
      <div className="flex justify-between text-slate-500"><span>Q1 (25%):</span> <span className="font-bold text-slate-800">{d.q1}</span></div>
      <div className="flex justify-between text-slate-500"><span>Thấp nhất (Min):</span> <span className="font-bold text-slate-800">{d.min}</span></div>
      <div className="flex justify-between text-slate-500 border-t pt-2 mt-2"><span>Trung bình (Mean):</span> <span className="font-bold text-red-600">{d.mean}</span></div>
    </div>
  );
}